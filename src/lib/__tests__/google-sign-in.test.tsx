...
it('trades a provider token for a session', async () => {
    ...
    await act(async () => {
        await result.current.signInWithGoogle('a-google-token');
    });
...
it('stops on the second factor instead of adopting a session', async () => {
    ...
    await act(async () => {
        outcome = await result.current.signInWithGoogle('a-google-token');
    });
